...
infinite-scroll      # Lista com carregamento automático ao rolar (IntersectionObserver)
po-tree              # Navegação hierárquica com po-tree-view (flat-to-tree ou lazy loading)
...
action-list          # Lista com N ações procedurais — modal confirm, loading por botão, resultados parciais
...
http-interceptor     # Interceptor funcional — auth token, tradução de erros, loading overlay
route-guard          # Guard funcional — CanActivate (auth/permissão) e CanDeactivate
standalone-migrate   # Migra componente legado NgModule → standalone + OnPush + signals
upload               # Upload de arquivos com po-upload (único, múltiplo ou embutido em form)
auth-login           # Tela de login (po-page-login + AuthService + authGuard + tokenInterceptor)
```
...
# Migrar componente legado para standalone
/poui-specialist:generate standalone-migrate ClientesLegado --module cadastro

# Upload de arquivos
/poui-specialist:generate upload Anexos --module compras
```
...
3. **Confirmação** — lista os arquivos criados com seus caminhos absolutos e rota sugerida
4. **Verificação de build** — compila automaticamente com `ng build` e corrige erros TypeScript/template sem intervenção ([`/poui-specialist:build-fix`](/docs/otimizacao-tokens#verificacao-de-build-automatica))
5. **Preview no browser** — pergunta se deseja visualizar o componente gerado (opcional)
