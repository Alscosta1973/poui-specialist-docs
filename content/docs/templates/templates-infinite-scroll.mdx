...
<po-page-list
  ...
  <!-- `[p-loading]` já exibe o overlay de carregamento embutido no po-table —
       não existe um elemento `<po-loading-overlay>` separado na API do PO-UI. -->
  <po-table
    [p-columns]="columns"
    [p-items]="items()"
    [p-loading]="loading()"
  ...
  <!-- Fim da lista -->
  @if (!loading() && !hasNext() && items().length > 0) {
