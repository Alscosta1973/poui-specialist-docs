...
| `refactor` | Converte ADVPL existente para PO-UI |
| `http-interceptor` | Interceptor funcional — auth token, tradução de erros, loading overlay |
| `route-guard` | Guard funcional — CanActivate e CanDeactivate |
| `standalone-migrate` | Migra componente legado NgModule → standalone + OnPush + signals |
| `upload` | Upload de arquivos com po-upload |
| `auth-login` | Tela de login com po-page-login + AuthService + authGuard |
